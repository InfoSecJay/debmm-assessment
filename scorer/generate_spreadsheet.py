...
def build_cover_tab(wb: Workbook):
    """First-sheet executive cover with engagement metadata pulled from the
    Assessment tab so the cover stays in sync as the user fills in details."""
    ws = wb.active
    ws.title = "Cover"
    ws.sheet_properties.tabColor = DARK_NAVY

    # Hide gridlines so the cover looks like a deliverable, not a worksheet
    ...
    for col in "ABCD":
        ws[f"{col}2"].fill = FILL_DARK_NAVY
    ...
    for col in "ABCD":
        ws[f"{col}3"].fill = FILL_NAVY

    ws.merge_cells("A4:D4")
    ws.row_dimensions[4].height = 14
    for col in "ABCD":
        ws[f"{col}4"].fill = FILL_NAVY

    # ── Metadata grid (2 x 3) ─────────────────────────────────────────
    LABEL_FONT = Font(name=FN, size=9, bold=True, color=MED_TEXT)
    VALUE_FONT = Font(name=FN, size=14, bold=True, color=DARK_TEXT)

    metadata_pairs = [
        # (label_left, value_left, label_right, value_right)
        ("PREPARED FOR", "=Assessment!D4", "ASSESSMENT DATE", "=Assessment!D7"),
        ("PREPARED BY", "=Assessment!D5", "VERSION", "1.0"),
        ("ASSESSOR ROLE", "=Assessment!D6", "CLASSIFICATION", "Confidential — Internal Use Only"),
    ]

    row = 6
    for left_label, left_value, right_label, right_value in metadata_pairs:
        # Labels row
        ws.cell(row=row, column=2, value=left_label)
        style_cell(ws.cell(row=row, column=2), LABEL_FONT, FILL_WHITE,
                   Alignment(horizontal="left", vertical="bottom"))
        ws.cell(row=row, column=3, value=right_label)
        style_cell(ws.cell(row=row, column=3), LABEL_FONT, FILL_WHITE,
                   Alignment(horizontal="left", vertical="bottom"))
        ws.row_dimensions[row].height = 16

        # Values row directly below
        ws.cell(row=row + 1, column=2, value=left_value)
        style_cell(ws.cell(row=row + 1, column=2), VALUE_FONT, FILL_WHITE,
                   Alignment(horizontal="left", vertical="center"))
        ws.cell(row=row + 1, column=3, value=right_value)
        style_cell(ws.cell(row=row + 1, column=3), VALUE_FONT, FILL_WHITE,
                   Alignment(horizontal="left", vertical="center"))
        ws.row_dimensions[row + 1].height = 24

        # Format date cell so it doesn't show the Excel serial
        if left_label == "PREPARED FOR" and right_label == "ASSESSMENT DATE":
            ws.cell(row=row + 1, column=3).number_format = "yyyy-mm-dd"

        # Hairline separator below the value row
        for col in "BC":
            ws[f"{col}{row + 1}"].border = HAIRLINE_BOTTOM

        row += 3  # label + value + spacer

    # ── About this report ─────────────────────────────────────────────
    row += 1
    ws.merge_cells(f"B{row}:C{row}")
    ws.cell(row=row, column=2, value="About this report")
    style_cell(ws.cell(row=row, column=2), FONT_SECTION, FILL_LIGHT_BLUE,
               ALIGN_LEFT, BLUE_ACCENT_LEFT)
    ws.cell(row=row, column=3).fill = FILL_LIGHT_BLUE
    ws.row_dimensions[row].height = 26
    row += 1

    about_lines = [
        "This workbook applies Elastic's Detection Engineering Behavior Maturity Model "
        "(DEBMM) to a single team. 24 criteria across 7 categories are scored on the "
        "1–5 maturity scale. The achieved tier is the highest tier whose criteria "
        "(and all lower tiers) score ≥ 3.0.",
        "",
        "Source: rubric/rubric.yaml · questionnaire/questionnaire.yaml. Generated by "
        "scorer/generate_spreadsheet.py. Re-fill the Assessment tab and re-save in Excel "
        "to refresh every other tab.",
    ]
    for line in about_lines:
        ws.merge_cells(f"B{row}:C{row}")
        ws.cell(row=row, column=2, value=line)
        style_cell(ws.cell(row=row, column=2), FONT_BODY, alignment=ALIGN_WRAP)
        ws.row_dimensions[row].height = 32 if line else 8
        row += 1

    # ── References footer ─────────────────────────────────────────────
    row += 1
    ws.merge_cells(f"B{row}:C{row}")
    ws.cell(row=row, column=2, value="References")
    style_cell(ws.cell(row=row, column=2), FONT_SECTION_TEAL, FILL_LIGHT_TEAL,
               ALIGN_LEFT, TEAL_ACCENT_LEFT)
    ws.cell(row=row, column=3).fill = FILL_LIGHT_TEAL
    ws.row_dimensions[row].height = 26
    row += 1
    ...
    for line in ref_lines:
        ws.merge_cells(f"B{row}:C{row}")
        ws.cell(row=row, column=2, value=line)
        style_cell(ws.cell(row=row, column=2), FONT_SMALL, alignment=ALIGN_WRAP)
        ws.row_dimensions[row].height = 16
        row += 1

    # Print setup — fit cover content to a single landscape page so the
    # references never orphan onto a second sheet on print.
...
def build_instructions_tab(wb: Workbook):
    ...
    def section_card(title, lines, accent="blue"):
        ...
        for line in lines:
            ...
            ws.cell(row=row, column=2, value=line)
            style_cell(ws.cell(row=row, column=2), FONT_BODY, alignment=ALIGN_WRAP)
            # Size row to the wrapped content. The merged B:C is ~90 chars wide
            # at the configured column widths; lines longer than that wrap and
            # get clipped in print if the row height isn't increased.
            if not line:
                ws.row_dimensions[row].height = 8
            else:
                wrapped_lines = max(1, (len(line) + 89) // 90)
                ws.row_dimensions[row].height = max(18, wrapped_lines * 16)
            row += 1
    ...
    section_card("Question Types & Scoring", [
        "Two dropdown question types are used:",
        "",
        "Scale (1–5): the answer IS the score directly. Each option is anchored to the rubric.",
        "Checklist (Yes/No): Yes maps to the maturity level the question represents (typically "
        "3.0 for foundational practices and 4.0 for higher-tier practices). No always maps to 1.0.",
        "",
        "Auditor note: the Yes-value is set per-question, not uniform — the same Yes can signal "
        "different maturity depending on what's being asked. The mapping is in questionnaire.yaml "
        "under scoring.yes_value for each checklist question.",
    ])
    ...
    # Print setup — landscape, fit content to one printed page wide so the cards
    # don't split across left/right page halves on export.
    ws.page_setup.orientation = ws.ORIENTATION_LANDSCAPE
    ws.page_setup.fitToWidth = 1
    ws.page_setup.fitToHeight = 0  # allow vertical paging across multiple pages
    ws.sheet_properties.pageSetUpPr.fitToPage = True
...
def build_assessment_tab(wb: Workbook, questionnaire: dict, rubric: dict):
    ...
    for q in questionnaire["questions"]:
        ...
        # Row height
        # Scale questions inline 5 level anchors; the question text plus all five
        # anchors needs ~150pt at the current column width to ensure levels 4 and 5
        # are not clipped in the printed export.
        ws.row_dimensions[row].height = 150 if qtype == "scale" else 35
    ...
    # Print setup — landscape, fit width so each question row prints with all
    # five level anchors visible (avoiding the prior anchor-truncation issue).
    ws.page_setup.orientation = ws.ORIENTATION_LANDSCAPE
    ws.page_setup.fitToWidth = 1
...
# ── Tab 4: Maturity Profile (radar) ─────────────────────────────


def build_maturity_chart_tab(wb: Workbook, core_tier_row_list: list, enrich_row_list: list):
    """Single radar chart spanning all 5 DEBMM core tiers + 2 enrichment dimensions
    on one canvas. Replaces the prior two bar-chart tabs (which used Excel's
    default cornflower blue and showed magnitude rather than capability shape).
    """
    if not core_tier_row_list and not enrich_row_list:
        return None

    ws = wb.create_sheet("Maturity Profile")
    ws.sheet_properties.tabColor = MED_BLUE

    col_widths = {"A": 3, "B": 26, "C": 12, "D": 12, "E": 3}
    for col, width in col_widths.items():
        ws.column_dimensions[col].width = width
    ...
    c = ws["A2"]
    c.value = "  Capability shape across DEBMM tiers and enrichment dimensions, against the 3.0 Defined threshold"
    style_cell(c, FONT_SUBTITLE, FILL_NAVY, Alignment(horizontal="left", vertical="center"))
    for col in "ABCDE":
        ws[f"{col}2"].fill = FILL_NAVY
    ...
    # Data table header
    row = 4
    headers = [("Dimension", FILL_STEEL), ("Score", FILL_STEEL), ("Threshold", FILL_STEEL)]
    for col_idx, (label, fill) in enumerate(headers, start=2):
        ws.cell(row=row, column=col_idx, value=label)
        style_cell(ws.cell(row=row, column=col_idx), FONT_COL_HEADER, fill, ALIGN_CENTER, THIN_BORDER)
    ws.row_dimensions[row].height = 24
    row += 1
    ...
    for name, trow in core_tier_row_list:
        ...
        ws.cell(row=row, column=4).number_format = "0.00"
        style_cell(ws.cell(row=row, column=4), FONT_SMALL_ITALIC, FILL_OFF_WHITE, ALIGN_CENTER, THIN_BORDER)
    ...
    chart.type = "marker"
    chart.style = 26
    chart.title = "Capability shape vs. 3.0 threshold"

    score_ref = Reference(ws, min_col=3, min_row=data_start - 1, max_row=data_end)
    threshold_ref = Reference(ws, min_col=4, min_row=data_start - 1, max_row=data_end)
    cats_ref = Reference(ws, min_col=2, min_row=data_start, max_row=data_end)

    chart.add_data(score_ref, titles_from_data=True)
    chart.add_data(threshold_ref, titles_from_data=True)
    chart.set_categories(cats_ref)

    # Make sure dimension names render around the polygon
    chart.x_axis.delete = False
    chart.y_axis.scaling.min = 0
    chart.y_axis.scaling.max = 5

    # Score series: brand cyan, thick line so the profile reads at a glance
    if len(chart.series) >= 1:
        org = chart.series[0]
        org.graphicalProperties.line.solidFill = MED_BLUE
        org.graphicalProperties.line.width = 36000  # ~4.5pt
    # Threshold series: amber dashed ring so it contrasts with both the score line
    # AND the chart gridlines (a muted grey would blend into the gridlines)
    if len(chart.series) >= 2:
        target = chart.series[1]
        target.graphicalProperties.line.solidFill = ANSWER_BORDER_COLOR
        target.graphicalProperties.line.dashStyle = "dash"
        target.graphicalProperties.line.width = 22000  # ~2.75pt

    # Compact size so the chart fits within one printed landscape page
    # alongside the data table above it.
    chart.width = 16
    chart.height = 11
    row += 1
    ws.add_chart(chart, f"B{row}")

    # Print setup — landscape, fit to one page in both directions so the chart
...
def generate_spreadsheet(
    ...
    build_maturity_chart_tab(wb, core_tier_row_list, enrich_row_list)
